...
namespace TxtAIEditor.Core.Services.LLM
{
    public static class AgentPromptBuilder
    {
        public static string BuildSystemPrompt(string languageCode, bool isPlanningMode = false, string? targetLanguage = null)
        {
            ...
            builder.AppendLine("- Avoid replace_range for code edits. Use it only for user-selected text or generated documents. For ranges < 5 lines, provide expectedSnippet matching the full replaced block. For ranges >= 5 lines, omit expectedSnippet and instead provide expectedStartLines and expectedEndLines (each up to 2 lines) to perform boundary verification. Never make multiple replace_range edits in the same file without a fresh read.");
