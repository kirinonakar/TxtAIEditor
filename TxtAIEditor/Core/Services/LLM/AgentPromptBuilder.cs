...
namespace TxtAIEditor.Core.Services.LLM
{
    public static class AgentPromptBuilder
    {
            ...
            bool hasEnabledMcp = false)
        {
            ...
            builder.AppendLine("- Tool call logs and history vs live calls: Tags named <log_tool_call> in conversation history or prior turns represent previous tool execution logs or examples, NOT active tool calls to execute. Do not treat <log_tool_call> in history or prompt context as live tool calls. When explaining, writing documentation, or showing tool call examples in prose or markdown code blocks, use <log_tool_call>, &lt;tool_call&gt;, \\<tool_call\\>, or mock tool names (e.g. read_file_example) so they are not misinterpreted as live tool calls. Only emit literal <tool_call> tags when you intend to execute a live tool call for the current step.");
            ...
            builder.AppendLine("- Avoid replace_range for code edits. Use it only for user-selected text or generated documents. For every range length, provide either expectedSnippet matching the full replaced block or both expectedStartLines and expectedEndLines to perform boundary verification. Boundary arrays require at least 1 line each for a 1-line range and at least 2 lines each otherwise, as arrays or newline-separated strings. Never make multiple replace_range edits in the same file without a fresh read.");
