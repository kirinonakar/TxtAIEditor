...
namespace TxtAIEditor.Core.Services.LLM
{
    public static class AgentPromptBuilder
    {
            ...
            bool hasEnabledMcp = false)
        {
            ...
            builder.AppendLine("- run_powershell usage: pass the PowerShell script body directly, for example run_powershell {\"command\":\"Get-Content -Raw -Encoding utf8 'README.md'\",\"timeoutMs\":10000}. The host uses PowerShell 7 when installed and otherwise forces UTF-8 console and file-cmdlet defaults in Windows PowerShell; when writing text is explicitly requested and necessary, still specify -Encoding utf8. Do not add a powershell -Command wrapper. If a command nevertheless invokes powershell or powershell.exe while PowerShell 7 is available, the host redirects that command name to the active pwsh.exe so it cannot fall back to Windows PowerShell 5.1.");
            builder.AppendLine("- For edits to user or project files, use the dedicated file edit tools (create_file, replace_in_file, search_replace, apply_patch, overwrite_file, insert_to_file, replace_range, append_to_file, merge_files, or split_file), not Set-Content. These edit tools preserve the file's encoding and LF/CRLF style and record the change in the edit ledger. Out-File, Add-Content, and shell redirection (> or >>) may be used only for command output, temporary diagnostics, and log files, not to edit user or project file content.");
            builder.AppendLine("- Preserve each existing file's exact line-ending style (LF or CRLF) and whether it has a final newline. If an explicitly requested or user-supplied PowerShell command uses Set-Content, read existing text with Get-Content -Raw and write it with Set-Content -NoNewline; the host automatically adds either missing switch as a safety fallback instead of failing the command.");
