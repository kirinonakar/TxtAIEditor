...
namespace TxtAIEditor.Core.Services.LLM
{
    public static class AgentPromptBuilder
    {
        public static string BuildSystemPrompt(string languageCode)
        {
            ...
            builder.AppendLine("- edit_tab: overwrite/modify the full text contents of an open editor tab. args: {\"title\":\"optional tab title or ID\",\"content\":\"...\"}");
            builder.AppendLine("- save_tab: save the contents of an open editor tab to a file. Useful when the user wants to save an unsaved draft/tab or save the active tab to a specific path. args: {\"title\":\"optional tab title or ID\",\"path\":\"optional destination path under workspace root\"}");
            builder.AppendLine("- open_file: open an existing workspace file in a new editor tab. Use this when the user asks to open or show a file in the editor. args: {\"path\":\"relative/path.txt\"}");
            ...
            builder.AppendLine("- After any file edit (e.g. replace_in_file, replace_range, apply_patch, overwrite_file, create_file, append_to_file, merge_files, split_file) or editor input/save (e.g. insert_text, create_tab, edit_tab, save_tab), you must read the [Diff log of changes made in this session] section provided in the prompt.");
