...
namespace TxtAIEditor.Core.Services.LLM
{
    public static class AgentPromptBuilder
    {
            ...
            bool hasEnabledMcp = false)
        {
            ...
            builder.AppendLine("- Avoid replace_range for code edits. Use it only for user-selected text or generated documents. For ranges < 5 lines, provide expectedSnippet matching the full replaced block. For ranges >= 5 lines, provide either expectedSnippet matching the full replaced block or expectedStartLines and expectedEndLines (each exactly 2 lines, as arrays or newline-separated strings) to perform boundary verification. Never make multiple replace_range edits in the same file without a fresh read.");
