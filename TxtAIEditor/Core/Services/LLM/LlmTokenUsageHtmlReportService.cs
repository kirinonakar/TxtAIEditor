...
namespace TxtAIEditor.Core.Services.LLM
{
    internal static class LlmTokenUsageHtmlReportService
    {
            ...
            Func<string, string, string> getString)
        {
            ...
            string html = template.Replace(DataPlaceholder, json, StringComparison.Ordinal);

            string reportDirectory = Path.Combine(SettingsBackupService.SettingsDirectoryPath, "reports");
            Directory.CreateDirectory(reportDirectory);
            string reportPath = Path.Combine(reportDirectory, "llm-token-usage.html");
